...
static async Task AndroidTvActionsAsync()
{
    ...
    using (var request = JsonDocument.Parse(requestHandler.Requests[3].Content!))
    {
        ...
        // Nineteen readable, distinct ids survive; the empty one and the repeat do not.
        Assert.Equal(19, input.GetProperty("externalIds").GetArrayLength());
        Assert.Equal("tvdb", input.GetProperty("externalIds")[0].GetProperty("source").GetString());
        Assert.Equal("7", input.GetProperty("externalIds")[0].GetProperty("value").GetString());
        Assert.Equal(JsonValueKind.Null, input.GetProperty("year").ValueKind);
